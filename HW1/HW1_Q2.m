...
T1Reward = 100; %
trialLimit = 200; % no more trial than it
moveLimit = 500; % no more movement than it within a trial
...
for trial = 1 : trialLimit
    ...
    for move = 1 : moveLimit
       ...
       if  move == moveLimit-1
        ...
        % Target move
       offset = targetLocation - agentLocation;
       [offset_max offset_i] = max(offset);
       newTargetDir = zeros(1,2);
       if offset == [0 0] break; end;
        newTargetDir(1,offset_i) = offset_max/abs(offset_max);
    
        if IfMoveable(targetLocation, newTargetDir, [worldX worldY]) == 1
            targetLocation = targetLocation + newTargetDir;
        else
            targetActionNum = agentActionNum;
            while targetActionNum == agentActionNum
